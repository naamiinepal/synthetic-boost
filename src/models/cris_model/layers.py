...
class TransformerDecoder(nn.Module):
    ...
    @staticmethod
    def pos1d(d_model: int, length: int):
        """
    ...
    @staticmethod
    def pos2d(d_model: int, height: int, width: int):
        """
        :param d_model: dimension of the model
...
_int_3_tup = Tuple[int, int, int]


class FPN(nn.Module):
    def __init__(
        self,
        in_channels: _int_3_tup = (512, 1024, 1024),
        out_channels: _int_3_tup = (256, 512, 1024),
    ):
    ...
    def forward(
        self, imgs: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], state: torch.Tensor
    ):
